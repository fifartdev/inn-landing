...
- **Sitemap**: dynamically generated by [app/sitemap.ts](app/sitemap.ts) — served at `/sitemap.xml`. All 10 routes are listed with `priority` and `changeFrequency`. Submit `https://innacademy.gr/sitemap.xml` to Google Search Console.
