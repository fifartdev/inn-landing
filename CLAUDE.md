...
- **Sitemap**: dynamically generated by [app/sitemap.ts](app/sitemap.ts) — served at `/sitemap.xml`. All 10 public routes are listed; `/thank-you` is intentionally excluded (it is `noindex`). Submit `https://innacademy.gr/sitemap.xml` to Google Search Console.
