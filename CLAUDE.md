...
- **Airtable lead logging**: After emails are sent the API fires a non-blocking `fetch` to `https://api.airtable.com/v0/{AIRTABLE_BASE_ID}/Leads`. Columns: Name, Phone, Email, Language, UTM Source, UTM Medium, UTM Campaign, GCLID, Timestamp. A failure never blocks the form response. Requires `AIRTABLE_TOKEN` and `AIRTABLE_BASE_ID` env vars — leave unset to disable.

### SEO

- **Sitemap**: dynamically generated by [app/sitemap.ts](app/sitemap.ts) — served at `/sitemap.xml`. All 10 public routes are listed; `/thank-you` is intentionally excluded (it is `noindex`). Submit `https://www.innacademy.gr/sitemap.xml` to Google Search Console.
...
- **OG image**: `public/innacademyfeatured.jpg` (1080×1080) is the shared OG image for all pages — set in the root layout and inherited by all sub-pages. Facebook, LinkedIn, Twitter/X, WhatsApp, Telegram, and Slack crawlers are explicitly allowed in `robots.txt`.
